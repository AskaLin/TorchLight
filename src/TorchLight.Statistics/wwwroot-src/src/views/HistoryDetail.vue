<template>
  <div class="history-detail">
    ...
    <div v-else-if="!historyData" class="error">
      <p>找不到歷史記錄資料</p>
      <p class="debug-info">請確認檔案是否存在</p>
      <button @click="goBack" class="btn-retry">返回列表</button>
    </div>
...
<script setup>
  ...
  const historyTitle = computed(() => {
    if (!historyData.value || !historyData.value.records || historyData.value.records.length === 0) {
      return '歷史記錄詳情'
    }
  ...
  const sortedRecords = computed(() => {
    if (!historyData.value || !historyData.value.records) return []
  ...
  onMounted(async () => {
    ...
    try {
      // 🔧 優先從 router state 獲取（如果有的話）
      if (route.state?.historyData) {
        historyData.value = route.state.historyData
        console.log('Loaded history data from router state:', historyData.value)
      } else if (route.query.fileName) {
        // 🔧 如果沒有 state，從後端重新加載
        console.log('Loading history data from backend for file:', route.query.fileName)
        const detail = await apiCall('GetHistoryRecordDetail', route.query.fileName)

        if (detail && !detail.error) {
          historyData.value = detail
          console.log('Loaded history data from backend:', historyData.value)
        } else {
  ...
  const viewMapDetail = (record) => {
    ...
    router.push({
      ...
      query: {
        fromHistory: 'true',
        fileName: route.query.fileName  // 傳遞 fileName 以便返回
      },
      state: {
        historyRecord: record,  // ✅ 傳遞單個記錄
        fromHistoryList: true,
        historyData: historyData.value  // ✅ 傳遞完整歷史數據（作為備用）
      }
    })
  }
